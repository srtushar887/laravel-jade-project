...
@section('user')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Close Request</h4>
                    ...
                    <div class="table-responsive m-t-40">
                        <table id="example23"
                            ...
                            <tbody>
                            @foreach($close_request as $clreqi)
                                <tr>
                                    <td>{{$clreqi->property->property_name}}</td>
                                    <td>{{$clreqi->request_title}}</td>
                                    <td>
                                        @if($clreqi->sel_priority == 1)
                                            Normal
                                        @elseif($clreqi->sel_priority == 2)
                                            Important
                                        @else
                                            Urgent
                                        @endif
                                    ...
                                    <td>{{$clreqi->request_des}}</td>
                                    <td>
                                        @if($clreqi->status == 0)
                                            Submited
                                        @elseif($clreqi->sel_priority == 1)
                                            Inprocess
                                        @elseif($clreqi->sel_priority == 1)
                                            Complete
                                        @else
                                    ...
                                    <td>{{$clreqi->created_at}}</td>
                                    <td>{{$clreqi->updated_at}}</td>
                                </tr>
                            @endforeach
